...
**Domain:** Single-user cycling form & performance analyzer (CV pose estimation + FIT telemetry + GCP ML)
**Researched:** 2026-05-20
**Confidence:** HIGH (most choices verified against current PyPI / official docs; medium confidence on Vertex AI sub-product selection)
...
Confidence: HIGH on MediaPipe, OpenCV, Polars, Garmin SDK, GCS, BigQuery, Streamlit versions (all verified against PyPI in May 2026). MEDIUM on Vertex AI SDK necessity — depends on whether you want to add a Vertex AI Pipelines flourish in a later phase.
...
Result: `pyproject.toml` + `uv.lock` committed to the repo. `uv sync` reproduces the env anywhere (including the Dockerfile).
...
---
*Stack research for: cycling form & performance analyzer (CV + telemetry + GCP)*
*Researched: 2026-05-20*
